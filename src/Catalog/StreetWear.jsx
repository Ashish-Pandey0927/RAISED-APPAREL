...
const StreetWear = () => {
  const { category } = useParams(); // Get category from URL (though not directly used for URL construction here, but for fetching)
  ...
  useEffect(() => {
    fetch("/products.json") // Ensure the correct path
      ...
      .then((data) => {
        if (data["athleisure"]) {
          setProducts(data["athleisure"]); // Load only the athleisure category
        } else {
          setProducts([]);
        }
      })
      .catch((error) => console.error("Error loading products:", error));
  }, [category]); // category in dependency array for potential reuse
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          {"athleisure"} Collection {/* Updated header to "Athleisure" to match data key */}
        </h2>
        ...
        <div style={{
          ...
          {products.map((product) => (
            <div
              ...
              onClick={() => navigate(`/athleisure/product/${product.id}`)} 
              // {/* **Modified navigate here!** - using "streetwear" as category in URL */}
            >
              <img
