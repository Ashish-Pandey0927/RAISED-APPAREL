...
const WomenswearCatalog = () => {
  const { category } = useParams(); // Get category from URL (though not directly used here anymore for URL construction but for fetching)
  ...
  useEffect(() => {
    fetch("/products.json") // Ensure the correct path
      ...
      .then((data) => {
        if (data["womenswear"]) {
          setProducts(data["womenswear"]); // Load only the selected category
        } else {
          setProducts([]);
        }
      })
      .catch((error) => console.error("Error loading products:", error));
  }, [category]); // category is in dependency array because component might be reused for different categories later
        ...
        >
          {products.map((product) => (
            <div
              ...
              onClick={() => navigate(`/womenswear/product/${product.id}`)}
            >
              <img
                ...
                style={{
                  width: "100%",
                  borderRadius: "0.5rem",
                  // aspectRatio: "1/1",  // Commenting out aspectRatio to test
                  objectFit: "cover",
                }}
              />
              <h3
